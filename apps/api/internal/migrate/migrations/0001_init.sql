...
CREATE TABLE enrollment_codes (
    ...
    name           text NOT NULL DEFAULT '',
    remaining_uses integer NOT NULL DEFAULT 1 CHECK (remaining_uses BETWEEN 1 AND 1000),
    created_at     timestamptz NOT NULL DEFAULT now(),
...
-- self-reported (OS/arch/available pilots) and used for run dispatch.
CREATE TABLE rovers (
    id                  bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    public_id           uuid NOT NULL DEFAULT gen_random_uuid(),
    fleet_id            bigint NOT NULL,
...
CREATE TRIGGER rovers_changed_trigger
    AFTER INSERT OR UPDATE OF name, units, tags, auto_tags, last_seen_at OR DELETE
    ON rovers
