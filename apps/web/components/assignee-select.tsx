...
export function CrewOption({ crew, crewIcon = "icon" }: { crew: Crew; crewIcon?: "emoji" | "icon" }) {
  ...
  return (
    <span className="flex min-w-0 items-center gap-2">
      ...
      {visible.length > 0 && (
        <span title={pilots.map((m) => `${pilotLabel(m.member_id)}${m.role === "captain" ? " (captain)" : ""}`).join(", ")} className="ml-auto inline-flex shrink-0 items-center gap-0.5">
...
export function PilotOption({ kind, unavailable }: { kind: string; unavailable: boolean }) {
  return <span className="flex items-center gap-2"><PilotIcon kind={kind} /> {pilotLabel(kind)}{unavailable && " — no rover"}</span>;
}
