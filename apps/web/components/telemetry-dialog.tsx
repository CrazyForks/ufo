...
const TYPE_LABEL_KEY: Record<RunMessage["type"], MessageKey> = {
  thinking: "telemetry.thinking",
  text: "telemetry.pilot",
  tool_use: "telemetry.tool",
  tool_result: "telemetry.result",
      ...
      <DialogContent className="flex max-h-[80vh] w-full max-w-2xl flex-col">
        <DialogHeader className="mb-0">
          <DialogTitle className="flex items-center gap-2 pr-8 text-base">
            ...
            {t("telemetry.runTitle", { id: run?.id ?? "" })}
            {duration && <span className="ml-auto text-xs font-normal tabular-nums text-muted-foreground">{duration}</span>}
          </DialogTitle>
        </DialogHeader>

        {(() => {
          const usageLine = formatRunUsage(liveRun?.usage, t);
          return usageLine ? (
            <p className="mt-1 text-xs tabular-nums text-muted-foreground">
              {t("telemetry.costLine", { usage: usageLine })}
            </p>
          ) : null;
        })()}
        ...
        <div className="-mr-2 mt-3 flex-1 space-y-2.5 overflow-y-auto pr-2">
          {visibleMsgs.length === 0 && (
            <p className="text-sm text-muted-foreground">{msgs.length > 0 && !telemetryShowAll ? t("telemetry.noReplies") : active ? t("telemetry.waiting") : t("telemetry.noForRun")}</p>
          )}
          {telemetryShowAll && events.length > 0 && (
            <div className="space-y-1.5 rounded-md border border-border bg-muted/30 p-2">
              <p className="text-xs font-semibold text-muted-foreground">{t("telemetry.runEvents")}</p>
          ...
          {telemetryShowAll && active && (
            <div className="flex items-center gap-1.5 text-xs text-info"><Loader2 className="size-3 animate-spin" /> {t("telemetry.running")}</div>
          )}
...
function ArtifactBlock({ artifact }: { artifact: Artifact }) {
  ...
  return (
    <div>
      <div className="flex items-center gap-1.5">
        ...
        {truncated && (
          <button type="button" className="text-[11px] text-muted-foreground hover:text-foreground" onClick={loadFullContent} disabled={loading}>
            {loading ? t("telemetry.loading") : t("telemetry.loadFull")}
          </button>
        )}
        {!artifact.asset_id && (
          <>
            <button type="button" className="text-muted-foreground hover:text-foreground disabled:opacity-50" title={copied ? t("telemetry.copied") : t("common.copy")} aria-label={`${t("common.copy")} ${artifact.name}`} disabled={loading} onClick={copyContent}>
              {copied ? <Check className="size-3.5" /> : <Copy className="size-3.5" />}
            </button>
            <a href={contentPath} download={artifact.name || "artifact.txt"} className="text-muted-foreground hover:text-foreground" title={t("common.download")} aria-label={`${t("common.download")} ${artifact.name}`}>
              <Download className="size-3.5" />
            </a>
          </>
        )}
        {artifact.asset_id && (
          <a href={assetFilePath(artifact.asset_id)} className="text-muted-foreground hover:text-foreground" title={t("common.download")} aria-label={`${t("common.download")} ${artifact.name}`}>
            <Download className="size-3.5" />
          </a>
        )}
      </div>
      ...
      {failed && <p className="mt-1 text-xs text-destructive">{t("telemetry.loadFailed")}</p>}
      {copyFailed && <p className="mt-1 text-xs text-destructive">{t("telemetry.copyFailed")}</p>}
    </div>
  );
}
  ...
  return (
    <div className="ufo-run-log-bar mt-3 flex h-2.5 gap-0.5 overflow-hidden rounded">
      {segs.map((s) => (
        <button
          key={s.sequence}
          title={`${t(TYPE_LABEL_KEY[s.type])}${s.count > 1 ? ` ×${s.count}` : ""}`}
...
function TelemetryRow({ m, pilot, timeFormat }: { m: RunMessage; pilot?: string; timeFormat: TimeFormat }) {
  ...
  const label = m.type === "tool_use" ? ((m.tool ?? t("telemetry.tool")).replace(/^\w/, (c) => c.toUpperCase())) : m.type === "text" && pilot ? (
      ...
      <div className="min-w-0 flex-1">
        ...
        {m.type === "tool_use" && (
          <>
            ...
            {m.input && <Expandable label={t("telemetry.input")} body={JSON.stringify(m.input, null, 2)} />}
          </>
        )}
        {m.type === "tool_result" && (m.output ? <Expandable label={t("telemetry.output")} body={m.output} defaultOpen={m.output.length < 400} /> : <p className="text-xs text-muted-foreground">{t("telemetry.noOutput")}</p>)}
      </div>
    </div>
  );
}
